...
import com.test.aoner.fanow.test.dialog_flower.AddressSelectDialog_Tanz_flower;
...
public class AddressPickViewManager_Tanzan_flower extends BaseInfoInputViewManager_flower {
    ...
    public AddressPickViewManager_Tanzan_flower(View view, ProcessData_flower data,String pageTag){
    ...
    protected void init(){
        ...
        clickView.setOnClickListener(v -> {
            ...
            new AddressSelectDialog_Tanz_flower(mView.getContext(), inputTv, addressStrs -> {
                if (addressStrs.length<2) return;
                state = addressStrs[0];
                city = addressStrs[1];
                final long time2 = System.currentTimeMillis();
                HttpManager_flower.getInstance().saveUserBuriedPoint((pageTag+"_"+mData.getParamName()+"_CLICK").toUpperCase(),addressStrs[0]+"-"+addressStrs[1]+"$$"+(time2-time1));
            }).show();
        });
    ...
    public boolean checkInput() {
        ...
        return (!TextUtils.isEmpty(state))&&(!TextUtils.isEmpty(city));
    }
